...
def test_create_workout_success(client, auth_header, mocker):
    mocker.patch('app.services.workout_service.add_workout', return_value=None)
...
def test_get_workouts(client, auth_header, mocker):
    fake = [
        # mimic ORM objects
        type(
            'W', (object,), {'session_id': 's1', 'workout_name': 'A', 'date': '2025-01-01 00:00:00', 'exercises': []}
        )()
    ]
    mocker.patch('app.services.workout_service.get_user_workouts', return_value=fake)
    resp = client.get('/workout', headers=auth_header)
    assert resp.status_code == 200
    data = resp.get_json()
    assert isinstance(data, list) and data[0]['session_id'] == 's1'


def test_update_workout_validation(client, auth_header):
    # missing session_id
    resp = client.put('/workout', headers=auth_header, json={'workout_name': 'X', 'exercises': []})
    assert resp.status_code == 400


def test_update_workout_success(client, auth_header, mocker):
    mocker.patch('app.services.workout_service.update_workout_session', return_value=True)
    payload = {'session_id': 's2', 'workout_name': 'Push Day', 'exercises': [{'name': 'Bench', 'sets': 3, 'reps': 8}]}
    resp = client.put('/workout', headers=auth_header, json=payload)
    assert resp.status_code == 200
    assert 'updated successfully' in resp.get_json()['message']


def test_delete_workout_not_found(client, auth_header, mocker):
    mocker.patch('app.services.workout_service.remove_workout_session', return_value=False)
    resp = client.delete('/workout/s_not_exist', headers=auth_header)
    assert resp.status_code == 404


def test_delete_workout_success(client, auth_header, mocker):
    mocker.patch('app.services.workout_service.remove_workout_session', return_value=True)
