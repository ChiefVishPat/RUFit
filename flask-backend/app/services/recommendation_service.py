...
from app.models.userinfo import (GenderChoices, HeightUnits, TrainingGoals,
                                 TrainingIntensityLevels, Userinfo, WeightUnits)
...
BASE_WEIGHT_INCREMENT_LB = { TrainingIntensityLevels.AMATEUR: 2.5, TrainingIntensityLevels.EXPERIENCED: 5.0, TrainingIntensityLevels.PROFESSIONAL: 5.0 }
BASE_WEIGHT_INCREMENT_KG = { TrainingIntensityLevels.AMATEUR: 1.0, TrainingIntensityLevels.EXPERIENCED: 2.5, TrainingIntensityLevels.PROFESSIONAL: 2.5 }
...
class BMICategory(Enum):
    UNDERWEIGHT = "Underweight"; NORMAL = "Normal"; OVERWEIGHT = "Overweight"; OBESE = "Obese"; UNKNOWN = "Unknown"

class UserProfileClass(NamedTuple): # Represents the derived user class
    bmi_category: BMICategory; gender: GenderChoices
...
PROFILE_PROGRESSION_FACTOR = {
    ...
    (BMICategory.UNKNOWN, GenderChoices.MALE): 1.0, (BMICategory.UNKNOWN, GenderChoices.FEMALE): 0.8, (BMICategory.UNKNOWN, GenderChoices.OTHER): 1.0,
    (BMICategory.NORMAL, GenderChoices.OTHER): 1.0,
    # Males by BMI
    (BMICategory.UNDERWEIGHT, GenderChoices.MALE): 0.9, (BMICategory.NORMAL, GenderChoices.MALE): 1.0, (BMICategory.OVERWEIGHT, GenderChoices.MALE): 0.9, (BMICategory.OBESE, GenderChoices.MALE): 0.9,
    # Females by BMI
    (BMICategory.UNDERWEIGHT, GenderChoices.FEMALE): 0.9, (BMICategory.NORMAL, GenderChoices.FEMALE): 0.9, (BMICategory.OVERWEIGHT, GenderChoices.FEMALE): 0.9, (BMICategory.OBESE, GenderChoices.FEMALE): 0.85,
}

# --- Plateau Handling ---

class PlateauStrategy(Enum): # Defines plateau handling approaches
    REDUCE_WEIGHT_INC_REPS = "Reduce weight slightly, increase reps"; HOLD_WEIGHT_DEC_REPS_ADD_SET = "Keep weight, lower reps/set, add a set"; CHANGE_REP_RANGE = "Shift target rep range (e.g., to hypertrophy)"; DELOAD = "Perform a deload session"; SUGGEST_ALTERNATIVE = "Try an alternative exercise"; NONE = "No specific strategy applied yet"

PLATEAU_STRATEGY_CYCLE = [ PlateauStrategy.REDUCE_WEIGHT_INC_REPS, PlateauStrategy.HOLD_WEIGHT_DEC_REPS_ADD_SET, PlateauStrategy.CHANGE_REP_RANGE, PlateauStrategy.DELOAD, PlateauStrategy.SUGGEST_ALTERNATIVE]
...
def _get_alternative_exercise(exercise_name: str) -> Optional[str]:
    ...
    if primary_group in temp_muscle_group_map:
        for candidate_name in temp_muscle_group_map[primary_group]:
            if candidate_name.lower() != exercise_name.lower(): candidates.add(candidate_name)
...
def _get_user_profile_class(user_info: Userinfo) -> UserProfileClass:
    ...
    try:
        ...
        else: raise ValueError(f"Invalid weight unit or value: {user_info.weight} {user_info.weight_unit}")

        height_m: Optional[float] = None
        if user_info.height_unit == HeightUnits.US and user_info.height_ft is not None and user_info.height_in is not None:
             total_inches = (user_info.height_ft * 12) + user_info.height_in
             height_m = total_inches * 0.0254
        elif user_info.height_unit == HeightUnits.SI and user_info.height_ft is not None: # Assuming SI height stored in cm in height_ft
             if user_info.height_ft > 0: height_m = user_info.height_ft / 100.0
             else: raise ValueError("Invalid height value for SI unit (assuming cm)")
        else: raise ValueError(f"Invalid height unit or value: {user_info.height_unit}")
        ...
        if bmi < 18.5: bmi_category = BMICategory.UNDERWEIGHT
        elif 18.5 <= bmi < 25: bmi_category = BMICategory.NORMAL
        elif 25 <= bmi < 30: bmi_category = BMICategory.OVERWEIGHT
        else: bmi_category = BMICategory.OBESE

        logger.debug(f"User {user_info.user_id}: BMI={bmi:.2f}, Category={bmi_category.name}, Gender={user_info.gender.name}")
...
def _format_weight_string(weight: float, unit: WeightUnits) -> str:
    if weight < 0.1: return "Bodyweight" # Threshold handles potential float inaccuracies
    else: return f"~{weight:.1f} {unit.value}"
...
def _get_next_target(
    ...
    if weight_unit == WeightUnits.LB: base_weight_inc = BASE_WEIGHT_INCREMENT_LB.get(intensity, 5.0)
    else: base_weight_inc = BASE_WEIGHT_INCREMENT_KG.get(intensity, 2.5)
...
def calculate_recommendations(user_id: str) -> Dict:
    ...
    for w in all_workouts: exercises_done[w.exercise].append(w)
    ...
    for exercise, history in exercises_done.items():
        ...
        if len(history) < 2:
            last = history[-1]
            recommendations[exercise] = {'sets': last.sets, 'reps': last.reps, 'weight': last.weight, 'notes': 'Log this exercise again to get your first recommendation.'}
            continue
        ...
        if is_plateau:
            ...
            except ValueError: logger.warning(f"Unknown plateau strategy '{last_strategy}'. Starting cycle.")
            ...
            if next_strategy == PlateauStrategy.REDUCE_WEIGHT_INC_REPS:
                target_weight = round(last_workout.weight * 0.95, 2); target_reps = current_rep_range[1]; weight_str = _format_weight_string(target_weight, weight_unit)
                rec = {'sets': last_workout.sets, 'reps': target_reps, 'weight': target_weight, 'notes': f"Plateau Strategy: Try reducing weight to {weight_str} and aim for {target_reps} reps."}
            elif next_strategy == PlateauStrategy.HOLD_WEIGHT_DEC_REPS_ADD_SET:
                target_reps = current_rep_range[0]; target_sets = last_workout.sets + 1; weight_str = _format_weight_string(last_workout.weight, weight_unit)
                rec = {'sets': target_sets, 'reps': target_reps, 'weight': last_workout.weight, 'notes': f"Plateau Strategy: Keep weight at {weight_str}, but do {target_sets} sets aiming for {target_reps} reps."}
                ...
                if goal == TrainingGoals.SURPLUS and current_rep_range == INITIAL_REP_RANGES[TrainingGoals.SURPLUS]:
                    crr = HYPERTROPHY_REP_RANGE; target_reps = crr[0]; weight_str = _format_weight_string(last_workout.weight, weight_unit)
                    rec = {'sets': last_workout.sets, 'reps': target_reps, 'weight': last_workout.weight, 'notes': f"Plateau Strategy: Let's switch focus. Aim for {target_reps} reps in the {crr[0]}-{crr[1]} range at {weight_str}."}
                else: applied_strategy = False
            elif next_strategy == PlateauStrategy.DELOAD:
                 target_sets = max(1, int(last_workout.sets * DELOAD_FACTOR)); target_weight = round(last_workout.weight * DELOAD_FACTOR, 2); weight_str = _format_weight_string(target_weight, weight_unit)
                 rec = {'sets': target_sets, 'reps': last_workout.reps, 'weight': target_weight, 'notes': f"Plateau Strategy: Time for a deload. Reduce to {weight_str} for {target_sets} sets of {last_workout.reps} reps."}
            elif next_strategy == PlateauStrategy.SUGGEST_ALTERNATIVE:
                alt_exercise = _get_alternative_exercise(exercise)
                if alt_exercise: rec = {'sets': 3, 'reps': 10, 'weight': 0, 'notes': f"Plateau persists. Strategy: Try an alternative exercise like '{alt_exercise}' (e.g., 3 sets of 10)."}
                else: applied_strategy = False

            if not applied_strategy: # Fallback
                 target_sets = max(1, int(last_workout.sets * DELOAD_FACTOR)); target_weight = round(last_workout.weight * DELOAD_FACTOR, 2); weight_str = _format_weight_string(target_weight, weight_unit)
                 rec = {'sets': target_sets, 'reps': last_workout.reps, 'weight': target_weight, 'notes': f"Plateau Strategy Fallback: Deload. Reduce to {weight_str} for {target_sets} sets."}
            ...
            recommendations[exercise] = _get_next_target(last_workout, intensity, profile_class, current_rep_range, weight_unit)
            ...
            recommendations[exercise] = {'sets': last_workout.sets, 'reps': last_workout.reps, 'weight': last_workout.weight, 'notes': f"Repeat {last_workout.sets}x{last_workout.reps} at {weight_str}. Focus on excellent technique."}
