...
def create_workout():
    ...
    for ex in exercises:
        if not ex.get("name") or not ex.get("sets") or not ex.get("reps"):
            logger.warning(
                "Workout creation failed: Each exercise must include name, sets, and reps for user %s", user_id
            )
...
def update_workout(session_id):
    ...
    if not existing_rows:
        logger.warning(
            "Workout update failed: No workout session found for session_id %s and user %s", session_id, user_id
        )
    ...
    for ex in exercises:
        if not ex.get("name") or not ex.get("sets") or not ex.get("reps"):
            logger.warning(
                "Workout update failed: Each exercise must include name, sets, and reps for user %s", user_id
            )
            return jsonify({"message": "Each exercise must have a name, sets, and reps"}), 400
